...
@app.service_bus_queue_trigger(arg_name="inputMsg", queue_name="input", connection="ServiceBusConnection")
@app.service_bus_topic_output(arg_name="output", queue_name="output", connection="ServiceBusConnection")
def detect_objects(inputMsg: func.ServiceBusMessage, output: func.Out[str]):
    event_string = inputMsg.get_body().decode('utf-8')
